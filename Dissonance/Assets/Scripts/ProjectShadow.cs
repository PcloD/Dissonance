...
public class ProjectShadow : MonoBehaviour {
	...
	public void InitWithShadowMaterial(Material mat) {
		_shadowMaterial = mat;
	}
	...
	void Start () {
		...
		for (int i = 0; i < _planes.Length; i++) {
			GameObject shadowObject = new GameObject(gameObject.name + " " + _planes[i].gameObject.name + " Shadow");
			shadowObject.transform.parent = WorldManager.g.ShadowContainer;
			_shadowRenderers[i] = shadowObject.AddComponent<ShadowRenderer>();
